...
def backtest(symbol, agent_path, model_type="standard"):
    print(f"Backtesting {symbol} using {agent_path} ({model_type})...")
    today = datetime(2026, 2, 8)
    # Test on the last 2 years
    ...
    plt.savefig(f"backtest_{symbol}_{model_type}.png")
    print(f"Backtest finished. Final Net Worth: {net_worths[-1]:.2f}")
    print(f"Backtest results saved to backtest_{symbol}_{model_type}.png")
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        train_agent(symbol=args.symbol, episodes=args.episodes, source=args.source, model_type=args.model_type)
    else:
        # For backtest, we'll use yfinance by default but could also make it an arg
        backtest(args.symbol, args.model, model_type=args.model_type)